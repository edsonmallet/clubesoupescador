  ...
  return (
    <div className="flex gap-2">
      ...
      <div className="flex flex-1 flex-col gap-1">
        ...
        <p className="text-sm">
          {comment.deleted ? (
            <em className="text-brand-ink/40">[removido]</em>
          ) : (
            comment.body
          )}
        </p>
        ...
        {canReply && (
          <button
            ...
            className="w-fit text-xs text-brand-ink/60 hover:underline"
          >
            Responder
          </button>
        )}
        ...
        {showReply && (
          ...
          >
            <textarea
              ...
              rows={2}
              className="rounded-md border border-brand-ink/25 px-2 py-1 text-sm"
        ...
        {replies.length > 0 && (
          <div className="mt-2 flex flex-col gap-3 border-l border-brand-ink/15 pl-3">
            {replies.map((reply) => (
              <CommentCard
                key={reply.id}
                comment={reply}
